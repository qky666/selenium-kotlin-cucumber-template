...
abstract class CucumberTestNgTestRunnerBase: AbstractTestNGCucumberTests() {
    ...
    fun storeEnvironmentProperties(browser: String? = null, mobile: Boolean? = null) {
        ...
        val environmentPropertiesPath = if (allureResultsDirectory.endsWith("/")) "${allureResultsDirectory}environment.properties" else "${allureResultsDirectory}/environment.properties"
        ...
        environmentProperties.store(File(environmentPropertiesPath).outputStream(),"Environment values for Allure report")
    }
}